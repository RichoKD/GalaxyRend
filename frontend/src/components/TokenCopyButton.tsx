...
export function TokenCopyButton() {
  ...
  const token = getToken();
  const tokenPreview = token ? `${token.slice(0, 20)}...${token.slice(-20)}` : "No token available";
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        ...
        >
          ...
          <span className="hidden lg:inline text-xs font-medium">API Token</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px] max-w-[95vw] bg-zinc-900 border-zinc-800">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs sm:text-sm text-slate-400">
            Use this token to authenticate your API requests when placing orders.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3 sm:space-y-4 py-3 sm:py-4">
          <Alert className="bg-blue-500/10 border-blue-500/30">
            <AlertDescription className="text-xs sm:text-sm text-slate-300">
              <strong className="font-medium">Important:</strong> Keep this token secure. It grants access to your account.
            </AlertDescription>
          </Alert>

          <div className="space-y-2">
            <label className="text-xs sm:text-sm font-medium text-slate-300">Access Token</label>
          ...
          <div className="space-y-2 pt-1 sm:pt-2">
            <h4 className="text-xs sm:text-sm font-medium text-slate-300">How to use:</h4>
            <ol className="text-[10px] sm:text-xs text-slate-400 space-y-1 list-decimal list-inside">
              ...
              <li className="break-all">Header format: <code className="px-1.5 py-0.5 bg-zinc-950 rounded text-purple-400 font-mono">Authorization: Bearer YOUR_TOKEN</code></li>
            </ol>
          </div>
        </div>
      </DialogContent>
